...
const tipoInput = document.querySelector("[name=tipo]");
...
async function loadImages() {
  ...
  const telao = getImageBlob(`${type}/telao.png`);
  const story = getImageBlob(`${type}/story.png`);
...
async function drawData({ type = "telao", img }) {
  const coords = await getConfigJson();
  const department = tipoInput.value;

  const { rua: ruaCoords, bairro: bairroCoords } =
    coords[department][type] || {};

  if (!ruaCoords || !bairroCoords) {
    console.error("Oops, erro ao capturar coordenadas");
    return;
  }
